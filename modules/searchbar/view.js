...
const SearchbarView = Backbone.View.extend(
    /** @lends SearchbarView.prototype */ {
        ...
        initialize: function (config) {
            window.zoomToDesktopTopicTreeFolder =
                this.zoomToDesktopTopicTreeFolder;
            ...
            this.listenTo(this.model, {
                ...
                "change:placeholder change:buttonSearchTitle change:showAllResultsText":
                    this.initialRender,
            });
            ...
            if (config?.quickHelp === true || config?.quickHelp === false) {
                console.warn(
                    "The attribute 'Portalconfig.searchBar.quickHelp' is deprecated in the next major-release. Please use 'Portalconfig.quickHelp.configs.search'!"
                );
        ...
        toggleBtnQuestionColor: function () {
            if (store.getters["QuickHelp/active"]) {
                this.$("span.bootstrap-icon.question-icon").addClass(
                    "quickhelp-is-shown"
                );
            } else {
                this.$("span.bootstrap-icon.question-icon").removeClass(
                    "quickhelp-is-shown"
                );
            }
        },
        ...
        setSearchInputWidth: function () {
            if (
                $("#searchInput").closest(".collapse.navbar-collapse").length >
                    0 &&
                !isMobile()
            ) {
                this.$("#searchInput").width(
                    window.innerWidth - $(".desktop").width() - 160
                );
        ...
        menuLoaderReady: function (parentElementId) {
            ...
            if (this.model.get("initSearchString") !== undefined) {
                if (
                    this.model.get("isInitialRecommendedListCreated") === true
                ) {
                    this.renderRecommendedList();
        ...
        getDropdownHeight: function () {
            if (document.getElementById("masterportal-container")) {
                return (
                    document.getElementById("masterportal-container")
                        .offsetHeight - 130
                );
            }
            return 100;
        ...
        renderRecommendedList: function () {
            ...
            if (
                $("#table-nav-layers-panel").length > 0 &&
                $("#table-nav-layers-panel").hasClass("show")
            ) {
                // Upgrade to BT5
                const collapse = Collapse.getInstance(
                    $("#table-nav-layers-panel").get(0)
                );
            ...
            this.$("ul.dropdown-menu-search").html(
                this.templateRecommendedList(attr)
            );
            ...
            finalHitList.forEach(function (hit) {
                if (hit.additionalInfo !== undefined) {
                    if (hit.name.length + hit.additionalInfo.length > 50) {
                        ...
                        hit.additionalInfo = this.model.shortenString(
                            hit.additionalInfo,
                            20
                        );
                    }
        ...
        hitSelected: function (evt) {
            ...
            if (evt && Object.prototype.hasOwnProperty.call(evt, "cid")) {
                ...
                evt &&
                Object.prototype.hasOwnProperty.call(evt, "currentTarget") ===
                    true &&
            ...
            if (hit?.triggerEvent) {
                ...
                Radio.trigger(
                    hit.triggerEvent.channel,
                    hit.triggerEvent.event,
                    hit,
                    true,
                    evt?.handleObj?.type
                );
                ...
                // mobil
                    ...
                    Radio.trigger("ModelList", "addModelsByAttributes", {
                        id: hit.id,
                    });
                    Radio.trigger(
                        "ModelList",
        ...
        zoomToDesktopTopicTree: function (hitId, hitName) {
            const hitTyp = Radio.request("Parser", "getItemByAttributes", {
                    id: hitId,
                })?.typ?.toUpperCase(),
            ...
            if (layerTyp === "OBLIQUE" && mapMode !== "OBLIQUE") {
                ...
                (layerTyp === "TILESET3D" || layerTyp === "TERRAIN3D") &&
                mapMode !== "3D"
            ) {
         ...
         * @returns {void}
         */
        zoomToDesktopTopicTreeFolder: function (hitId) {
            Radio.trigger("ModelList", "openFolderInTree", hitId);
        },
        ...
        switchToObliqueMapMode: function (hitName) {
            store.dispatch("ConfirmAction/addSingleAction", {
                actionConfirmedCallback: () =>
                    Radio.trigger("Map", "mapChangeToOblique"),
                confirmCaption: "common:button.confirm",
                textContent: i18next.t(
                    "common:modules.searchbar.tree.chooseObliqueLayer",
                    { hitName }
                ),
            });
        },
        ...
        switchTo3dMapMode: function (hitId, hitName) {
            store.dispatch("ConfirmAction/addSingleAction", {
                ...
                confirmCaption: "common:button.confirm",
                textContent: i18next.t(
                    "common:modules.searchbar.tree.choose3dLayer",
                    { hitName }
                ),
            });
        },
        ...
        setMarkerZoom: function (hit) {
            ...
            if (
                ...
                if (store.getters.styleListLoaded) {
                    store.dispatch(
                        "MapMarker/placingPolygonMarker",
                        getWKTGeom(hit)
                    );
                    extent = store.getters["MapMarker/markerPolygon"]
                        .getSource()
                        .getExtent();
                    Radio.trigger("Map", "zoomToExtent", {
                        extent: extent,
                        options: { maxZoom: zoomLevel },
                    });
                } else {
                    store.watch(
                        ...
                        (value) => {
                            if (value) {
                                ...
                                extent = store.getters[
                                    "MapMarker/markerPolygon"
                                ]
                                    .getSource()
                                    .getExtent();
                                Radio.trigger("Map", "zoomToExtent", {
            ...
            if (evt.target.value.length === 0) {
                ...
                if (this.model.get("pasted")) {
                    ...
                    !(
                        ...
                        this.getSelectedElement("#searchInputUL").hasClass(
                            "type"
                        )
                    )
                ) {
        ...
        showMarker: async function (evt) {
            ...
            if (
                ...
                hit?.triggerEvent &&
                hit.type !==
                    i18next.t("common:modules.searchbar.type.subject") &&
                hit.type !==
                    i18next.t("common:modules.searchbar.type.general") &&
                hit.type !==
                    i18next.t("common:modules.searchbar.type.folder") &&
                hit.triggerEvent.event !== "gfiOnClick"
            ) {
                Radio.trigger(
                    hit.triggerEvent.channel,
                    hit.triggerEvent.event,
                ...
                hit?.type &&
                (hit.type ===
                    i18next.t("common:modules.searchbar.type.topic") ||
                    hit.type ===
                        i18next.t("common:modules.searchbar.type.subject") ||
                    hit.type ===
                        i18next.t("common:modules.searchbar.type.general") ||
        ...
        hideMarker: function (evt) {
            ...
            if (
                hit &&
                Object.prototype.hasOwnProperty.call(hit, "triggerEvent")
            ) {
                if (
                    hit.type !==
                        i18next.t("common:modules.searchbar.type.subject") &&
